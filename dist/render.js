...
function buildFallbackAvatar(student) {
...
function updateResultsOutput(visibleCount, totalCount) {
    const resultOutput = document.getElementById("results-count");
    if (!resultOutput) {
        return;
    }
...
function updateBioCount(currentCount, maxCount) {
    const bioOutput = document.getElementById("bio-count");
    if (!bioOutput) {
        return;
    }
